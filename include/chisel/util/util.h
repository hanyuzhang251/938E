 ...
 *
 * @note Originally made in bot two as a global definition #define wait(n) pros::delay(n) to make delaying more concise.
 *       If placed into the chisel namespace, it would exactly be shorter as chisel::wait(n).
 */
 ...
 *
 * @note Originally made in bot two as a global definition #define wait(n) pros::delay(n) to make delaying more concise.
 *             If placed into the chisel namespace, it would exactly be shorter as chisel::wait(n) so it was changed to a glokbal function.
 *             As chisel::wait(n) was already used in some places before the change, this function stays though is deprecated.
 */
[[deprecated]] inline void wait(const uint32_t delta) {
    pros::delay(delta);
}
